...
module.exports = function (seneca, opts, cb) {
  ...
  function configure(spec, cb) {
    ...
    conf.hosts     = spec.hosts || [spec.host + ':' + (spec.port || 9160)]
    conf.pass     = spec.password

    // Using PooledConnection has it recovers better than Connection
    var PooledConnection = cassandraClient.PooledConnection;
    var con = new PooledConnection(conf);
    con.connect(function(err) {
      ...
      dbinst = con
      dbinst.on('log', function(level, message, obj) {
        if ('error' === level) {
          seneca.log.error('log event: %s -- %j', level, message)
        }
        else if ('warn' === level) {
          seneca.log.warn('log event: %s -- %j', level, message)
        }
      });
      cb(err)
    })
    ...
    load: function (args, cb) {
      ...
      try{
        dbinst.execute(query.text, query.values, function (err, res) {
          if (!error(args, err, cb)) {
            if (res.rowCount() === 0) {return cb(null);}
            ...
            seneca.log.error({code: 'load', tag: args.tag$, store: store.name, query: query, error: err})
            return cb(err);
          }
        })
      }catch(err) {
      ...
      try{
        query = dbinst.execute(query.text, query.values, function (err, res) {
          if (!error(args, err, cb)) {
            ...
            seneca.log.error({code: 'list', tag: args.tag$, store: store.name, query: query, error: err})
            return cb(err);
          }
        })
      }catch(err){
        ...
        if (q.all$) {
          ...
          dbinst.execute(query.text, query.values, function (err) {
            if (!error(args, err, cb)) {
              ...
              seneca.log.error({code: 'remove', tag: args.tag$, store: store.name, query: query, error: err})
              return cb(err);
            }
          })
        } else {
  ...
  seneca.store.init(seneca, opts, store, function (err, tag, description) {
    ...
    configure(opts, function (err) {
      if (err) {
        seneca.log.error({code: 'entity/configure', store: store.name, error: err})
        return cb(err);
      }
